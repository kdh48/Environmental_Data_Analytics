...
```{r}
# Absolute file path is commented out so that when you try running the code and it's not the same comp it won't break
...
# Load package
library(tidyverse)
# library (rather than require) will tell you if there was an issue
```
...
You may also import datasets from the Files tab, but this is not recommended since this is not reproducible. #looking at knitting directory under dropdown to put it in a different working directory
...
tail(USGS.flow.data, 5) #date collected?
...
#Pay attention to what the class is, which can make a difference in what is imported with an n/a

# Tibbles is a way to separate into smaller chunks and get it to act together as a data frame, benefit is that working with tibbles is faster than using the whole data frame. R for Data Science book has more info on it.
```
