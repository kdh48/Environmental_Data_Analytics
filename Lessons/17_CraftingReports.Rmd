...
2. **R Code Chunks** surrounded by ``` on top and bottom
    + Create using `Cmd/Ctrl` + `Alt` + `I` (option command i)
...
-------------------|-------------------------------
EPA Air O3         | From NC 2017 & 2018
EPA PM2.5          | From NC 2017 & 2018
NTL-LTER Lakes     | Includes Nutrients & Chem
...
#Load data & set date format
Lakes.Nutrients.Raw<- read.csv("./Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv")

Lakes.Nutrients.Raw$sampledate <- as.Date(Lakes.Nutrients.Raw$sampledate, 
                                          format = "%Y-%m-%d")

#Check head
head(Lakes.Nutrients.Raw)
```
...
```{r, include = FALSE}
Lakes.Nutrients.Surface.TN_Sum <- 
  Lakes.Nutrients.Raw %>%
 ...
 Lakes.Nutrients.Surface.TP_Sum <- 
  Lakes.Nutrients.Raw %>%
...
```{r, echo=FALSE}
kable(Lakes.Nutrients.Surface.TN_Sum, caption = "Total Nitrogen Summary Table")

kable(Lakes.Nutrients.Surface.TP_Sum, caption = "Total Phosphorous Summary Table")
```

Create a fourth and fifth R chunk that generates two plots (one in each chunk): one for total N over time with different colors for each lake, and one with the same setup but for total P. Decide which geom option will be appropriate for your purpose, and select a color palette that is visually pleasing and accessible. Customize the chunk options such that the final figures are displayed but not the code used to generate the figures. < IGNORE: In addition, customize the chunk options such that the figures are aligned on the left side of the page. > Lastly, add a fig.cap chunk option to add a caption (title) to your plot that will display underneath the figure.
