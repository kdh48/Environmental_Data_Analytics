...
print(O3plot) #default is gray

O3plot1 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_gray()
print(O3plot1) #same as gray theme

O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_bw()
print(O3plot2) #black and white

O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light()
print(O3plot3) #very lightly lined axis and grid lines

O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic()d
print(O3plot4) #Kateri's favorite
```
...
#legend.position = c(1,1), legend.justification = c(1,1) would put the legend inside
#base font is size 12
...
theme_set(mytheme) #this will use this theme throughout the rest of the session so you don't have to type it in each time
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_rect(xmin = as.Date("2016-01-01"), xmax = as.Date("2020-01-01"), 
            ymin = 0, ymax = 50, fill = "green") + #this line is for shading the background of the green categorization of AQI, put up top because ggplot layers everything so if put below it would go above the points.
  geom_rect(xmin = as.Date("2016-01-01"), xmax = as.Date("2020-01-01"), 
            ymin = 50, ymax = 100, fill = "yellow") + #yellow categorization of AQI
  geom_point() +
  geom_text(x = as.Date("2019-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") + #adds corresponding label for green categorization
  geom_text(x = as.Date("2019-01-01"), y = 95, label = "moderate", hjust = 1, fontface = "bold") + #adds corresponding label for yellow categorization
    scale_x_date(limits = as.Date(c("2017-01-01", "2018-12-31")), #sets discrete range
    date_breaks = "2 months", date_labels = "%b %y") + #sets the labels to show month abbrev. and year, every 2 months. A capital B would have given the full month name. 
  ylab(expression("O"[3]* " AQI Value")) + #y label -- the brackets make the 3 a subscript, a ^ will superscript but it needs to be outside of the brackets
  theme(axis.text.x = element_text(angle = 45,  hjust = 1)) #overwrites the theme for just this element, angles the dates. hjust stands for horizontal justification, an h just of 1 moves things to the left.
print(O3plot7)  
...
NvsP2 <-
  ...
  labs(x = "Total Phosphorus, µg/L", y = "Total Nitrogen, µg/L", fill = "Depth, m", shape = "Lake Name") +
  scale_shape_manual(values = c(15, 17)) + #15 & 17 are solid shapes, change to 21, 23 to get outline
...
#Kateri's code for this one going on Slack
...
# place your additional edits here
  ...
  scale_color_manual(values = c("#7fcdbb", "#41b6c4", "#1d91c0", "#225ea8", "#0c2c84")) + #this uses same palette from the brewer YlGnBu but just took the darkest colors from it. Hex codes on website.
