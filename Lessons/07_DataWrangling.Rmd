...
---
Data wrangling is manipulating the data to fit our needs, cleaning it up, organizing it, reformatting, etc. It's helpful to do the exploration first so then you get a better idea of how to wrangle it -- ex. we noticed that one lake was sampled a lot more in the exploration stage. This may mean we're interested in some statistics just within that lake, so during the wrangling we could separate that lake out.  
...
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] #after the comma is where you tell it which columns to use, with nothing after it means all the columns

# dplyr filtering - tell it to use a datarame and then a conditional statement
...
head(NTL.phys.data.surface1) #matrix uses the row #s from original set
dim(NTL.phys.data.surface1)
head(NTL.phys.data.surface2) #dplyer renames the rows to be in order
...
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") #the | is the "or" operator. Need it to include.
...
#this removes the lakes, more helpful if you were just trying to remove one lake. Use & for removing items. 

NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))

# Choose a range of conditions of a numeric or integer variable. This is ignoring Leap Years
summary(NTL.phys.data$daynum)
NTL.phys.data.JunethruOctober1 <- filter(NTL.phys.data, daynum > 151 & daynum < 305)
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) #comma operates as an & here
...
NTL.phys.data.1999 <- filter(NTL.phys.data, year4 == "1999") #as a factor or character you need to put it in quotes, but as an integer we wouldn't have to use the quote. 

# what code do you need to use, based on the class of the variable?
# as a factor or character you need to put it in quotes, but as an integer we wouldn't have to use the quote. 
class(NTL.phys.data$year4)

# Exercise: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.Tues90_99 <- filter(NTL.phys.data, year4 >= 1990 & year4 <= 1999 & lakename == "Tuesday Lake")
...
East Long Lake & Hummingbird Lake 
...
#use commas and colons to use select, select is not as flexible as filter though. The colon is to be for the colums sampledate through temperature
...
#this is creating a new column that is a function of a different column
...
#if we used row.names = TRUE it would include the row numbers as the first column. One dot before hand means going down one, two dots means going up

#sometimes handy to comment out the #write.csv... so that you don't accidentally re-write a file if you don't mean to. 
```
