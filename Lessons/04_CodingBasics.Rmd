...
vector1[3] # gives you the 3rd item in vector 1. This is called matrix subsetting.

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # goes vertically, and then horizontal (default is by columns)
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) # goes horizontal, then vertical
matrix2
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line
                  dimnames = list(c("uno", "dos", "tres", "cuatro", "cinco"), 
                                  c("un", "deux", "trois", "cat"))) #
matrix3["uno",] #to pull up row, but comma after
matrix3[,"un"] #to pull up column, put comma before

matrix1[4, ] #pulls up row 4 across from matrix 1
matrix1[ , 3] #pulls up column 3 from matrix 1
matrix1[c(12, 14)] #this just pulls up elements 12 & 14
matrix1[c(12:14)] #with the colon, it pulls up 12 - 14
matrix1[2:4, 1:3] #pulling rows 2 - 4 and then column 1 - 3

#c stands for "concatenate" -- link or chain things together in a series
...
#if you highlight something, R will tell you where else in the doc it is

# Lists ---- 
list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3); list1
list1[[2]] #for lists we use double brackets. this will pull up the 2nd element (which is mynumbers)
...
dataframe1 <- data.frame(d,e,f) # This should show up under data, not values
...
> ANSWER: The two types are data and values, values is anythng with a c infront of it

QUESTION: In the R chunk below, write "dataframe1$". Press `tab` after you type the dollar sign. What happens?

> ANSWER: a pop up with the different column names pop up

QUESTION: What happens when a comment in R is followed by "----"?

> ANSWER: it creates a subsection that can be collapsed
