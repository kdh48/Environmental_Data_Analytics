...
plot(huc8_sf['SUB'],
     graticule = st_crs(4326), #Add a graticule #4326 is mercator projection
     axes=TRUE)

plot(huc8_sf['SUB'],
     graticule = st_crs(huc8_sf), #Switching to data name makes the same projection
     axes=TRUE)
...
leaflet() %>% 
  addProviderTiles(providers$Esri.NatGeoWorldMap) %>%  #(providers$Esri.WorldShadedRelief)
...
huc0302_counties_clip <- st_intersection(counties_sf_utm,huc0302_counties)
mapview(huc0302_counties_clip)
  
#Map the product, showing values by land area
mapview(huc0302_counties_clip['ALAND'])
```
...
#Early up in document st as sf? Might be on hmwk
