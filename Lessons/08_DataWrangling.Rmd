...
summary(NTL.phys.data.PeterPaul$comments) #the dissolved oxygen probe bad means that it couldn't calibrate to zero
...
NTL.nutrient.data.PeterPaul <- filter(NTL.nutrient.data, lakename == "Paul Lake" | lakename == "Peter Lake") #remember that | means "or" 
...
summary(NTL.nutrient.data.PeterPaul$comments) #this section wasn't filled with N/As because the class is factor
...
write.csv(NTL.nutrient.data.PeterPaul, row.names = FALSE, file = "./Data/Processed/NTL-LTER_Lake_Nutrients_PeterPaul_Processed.csv") #don't want tell R to save row #s in there own column -- ./ means go down one folder
...
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration)) #removing the n/as in concentration
...
NTL.nutrient.data.PeterPaul.dates <- separate(NTL.nutrient.data.PeterPaul.skinny, sampledate, c("Y", "m", "d")) #now the individual Y m & d are charachters in R, R only recognizes dates when all 3 are together. 
...
* `inner_join`: return rows in x where there are matching values in y, and all columns in x and y (mutating join). Very depent on the order you specify them
...
**Apparently this is a great package so try to explore it more
...
```{r} 
#this pipe means "then", depth == ) is only taking the surface depth, the filter !is.na code w/& means that it is removing the row if there is an n/a in any of those 3 (temp, total nitrogen, tp )
