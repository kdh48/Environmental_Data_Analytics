...
# another way to format the lm function, explanatory always on the right side
irradiance.regression <- lm(data = PeterPaul.chem.nutrients, irradianceWater ~ depth)
summary(irradiance.regression)
#a lot of variance because of low r squared, adjusted r squared is going to penalize you for adding more explanatory variables. We have 15449 degrees of freedom
...
> ANSWER: irradiance = 487 - 96(depth) + E. the irradiance is 486 and decreases by 96 for each unit of depth. Negatively correlated with depth.
...
#might want to remove the super outlier, or in this case, decided to zoom into the area w/the majority of the points by uncommenting the ylim.

# in this case, we don't want to use a linear regressin w/out log transforming after looking at the data
```
...
irradiancebydepth2 <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
  scale_y_log10() +
  geom_point(alpha = 0.5) + #this makes it 50% transparent
...
#se removes confidence interval
...
#the deeper we go, the more phosphorous. Total phosphorus = 6 + 1.5(depth) + .94(dissolvedoxygen) + error. Low p value but we're only explaining 8% of our data (from r squared). Not a good linear model for explaning phosphorous
...
PeterPaulnutrients <- 
  ...
  select(tn_ug:po4) %>%
  na.omit()
#selected only columns pertaining to the nutrients, then ommitted na's.
PeterPaulCorr <- cor(PeterPaulnutrients)
corrplot(PeterPaulCorr, method = "ellipse") #elipse provides a directional way of reading the data, without it would just be circle. size relates to size of the correlation
corrplot.mixed(PeterPaulCorr, upper = "ellipse")
#if you test too many variables you'll find some significant relationships that don't actually really correlate. Over parameterization of your model.

#not strong correlation between phosphate & total phosphorus which indicates that it has maybe more organic phosphorus in the lake
```
...
#response variable on the left, explanatory on the right. Way better model because 4 squared is better. 

#step automatically reduces the linear model for you by choosing the best one. 
...
#TN = 353 + 136 -97(depth). Still not a significant predictor.
