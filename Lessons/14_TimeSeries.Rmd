...
### Mann-Kendall Test
A Mann-Kendall test will analyze whether there is a monotonic trend in the response variable over time. Let's use the Mann-Kendall test to investigate whether there is a trend in total phosphorus concentrations in Peter Lake over time. *Does not account for seasonality
...
pettitt.test(Peter.nutrients.surface$tp_ug) #says 35
...
# z score is negative which shows a negative relationship?
...
#color matches Peter lake to show that it is correlated with that lake
...
#option to log axis w/out changing data to preserve units of measurement w/ scale_y_log10()
```
...
> ANSWER: Since discharge is continuous it would be either spline or linear. Kateri recommends linear in this case because spline uses a polynomial function and w/ecological data we don't really know what the polynomial function would be
...
USGS.flow.data.timeseries <- ts(USGS.flow.data.interpolated$Discharge, 
                                start = c(2009, 1) ,frequency = 12) #monthly, 365 for daily
